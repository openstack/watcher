...
class TestListAuditTemplate(FunctionalTestWithSetup):
    ...
    def test_one(self):
        ...
        response = self.get_json('/audit_templates')
        self.assertEqual(
            audit_template.uuid, response['audit_templates'][0]["uuid"]
        )
    ...
    def test_get_one_soft_deleted_ok(self):
        ...
        response = self.get_json(
            '/audit_templates', headers={'X-Show-Deleted': 'True'}
        )
        self.assertEqual(
            audit_template.uuid, response['audit_templates'][0]["uuid"]
        )
    ...
    def test_detail_soft_deleted(self):
        ...
        audit_template.soft_delete()
        response = self.get_json(
            '/audit_templates/detail', headers={'X-Show-Deleted': 'True'}
        )
        self.assertEqual(
            audit_template.uuid, response['audit_templates'][0]["uuid"]
        )
    ...
    def test_many(self):
        ...
        response = self.get_json('/audit_templates')
        self.assertEqual(
            len(audit_template_list), len(response['audit_templates'])
        )
        uuids = [s['uuid'] for s in response['audit_templates']]
        self.assertEqual(
            sorted([at.uuid for at in audit_template_list]), sorted(uuids)
        )
    ...
    def test_many_without_soft_deleted(self):
        ...
        uuids = [s['uuid'] for s in response['audit_templates']]
        self.assertEqual(
            sorted([at.uuid for at in audit_template_list[:3]]), sorted(uuids)
        )
    ...
    def test_filter_by_goal_uuid(self):
        for id_, goal_id in enumerate(
            itertools.chain.from_iterable(
                [
                    itertools.repeat(self.fake_goal1.id, 3),
                    itertools.repeat(self.fake_goal2.id, 2),
                ]
            ),
            1,
        ):
    ...
    def test_filter_by_strategy_uuid(self):
        for id_, strategy_id in enumerate(
            itertools.chain.from_iterable(
                [
                    itertools.repeat(self.fake_strategy1.id, 3),
                    itertools.repeat(self.fake_strategy2.id, 2),
                ]
            ),
            1,
        ):
    ...
    def test_many_with_sort_key_name(self):
        ...
        self.assertEqual(
            sorted([at.name for at in audit_template_list]), names
        )
    ...
    def test_many_with_sort_key_goal_name(self):
        ...
        for direction in ['asc', 'desc']:
            response = self.get_json(
                '/audit_templates?sort_key={}&sort_dir={}'.format(
                    'goal_name', direction
                )
            )
...
class TestPatch(FunctionalTestWithSetup):
    ...
    def test_replace_goal_uuid(self, mock_utcnow):
        ...
        response = self.patch_json(
            f'/audit_templates/{self.audit_template.uuid}',
            [{'path': '/goal', 'value': new_goal_uuid, 'op': 'replace'}],
        )
    ...
    def test_add_goal_uuid(self):
        response = self.patch_json(
            f'/audit_templates/{self.audit_template.uuid}',
            [{'path': '/goal', 'value': self.fake_goal2.uuid, 'op': 'add'}],
        )
        self.assertEqual('application/json', response.content_type)
    ...
    def test_remove_strategy(self):
        ...
        response = self.get_json(f'/audit_templates/{audit_template.uuid}')
...
class TestPost(FunctionalTestWithSetup):
    ...
    def test_create_audit_template(self, mock_utcnow):
        ...
        expected_location = '/v1/audit_templates/{}'.format(
            response.json['uuid']
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.fake_strategy1.uuid, response.json['strategy_uuid']
        )
    ...
    def test_create_audit_template_with_strategy_name(self, mock_utcnow):
        audit_template_dict = post_get_test_audit_template(
            goal=self.fake_goal1.uuid, strategy=self.fake_strategy1.name
        )
        test_time = datetime.datetime(2000, 1, 1, 0, 0)
        mock_utcnow.return_value = test_time
        ...
        self.assertEqual(
            self.fake_strategy1.name, response.json['strategy_name']
        )
        return_created_at = timeutils.parse_isotime(
            response.json['created_at']
        ).replace(tzinfo=None)
    ...
    def test_create_audit_template_validation_with_aggregates(self):
        scope = [
            {
                'compute': [
                    {'host_aggregates': [{'id': '*'}]},
                    {'availability_zones': [{'name': 'AZ1'}, {'name': 'AZ2'}]},
                    {
                        'exclude': [
                            {
                                'instances': [
        ...
        with self.assertRaisesRegex(
            webtest_app.AppError, "be included and excluded together"
        ):
        ...
        response = self.post_json(
            '/audit_templates', audit_template_dict, expect_errors=True
        )
    ...
    def test_create_audit_template_does_autogenerate_id(self):
        ...
        with mock.patch.object(
            ...
            response = self.post_json('/audit_templates', audit_template_dict)
        self.assertEqual(
            audit_template_dict['goal'], response.json['goal_uuid']
        )
...
class TestDelete(api_base.FunctionalTest):
    ...
    def test_delete_audit_template_by_uuid(self, mock_utcnow):
        ...
        at = objects.AuditTemplate.get_by_uuid(
    ...
    def test_delete_audit_template_by_name(self, mock_utcnow):
        ...
        self.context.show_deleted = True
        at = objects.AuditTemplate.get_by_name(
    ...
    def test_delete_audit_template_not_found(self):
        uuid = utils.generate_uuid()
        response = self.delete(f'/audit_templates/{uuid}', expect_errors=True)
        self.assertEqual(HTTPStatus.NOT_FOUND, response.status_int)
        self.assertEqual('application/json', response.content_type)
        self.assertTrue(response.json['error_message'])
...
class TestAuditTemplatePolicyEnforcement(api_base.FunctionalTest):
    ...
    def test_policy_disallow_create(self):
        ...
        audit_template_dict = post_get_test_audit_template(
            goal=fake_goal1.uuid, strategy=fake_strategy1.uuid
        )
        self._common_policy_check(
...
class TestAuditTemplatePolicyWithAdminContext(
    TestListAuditTemplate, api_base.AdminRoleTest
):
    def setUp(self):
        super().setUp()
        self.policy.set_rules(
