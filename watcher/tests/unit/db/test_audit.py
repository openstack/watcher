...
class TestDbAuditFilters(base.DbTestCase):
    ...
    def test_get_audit_list_filter_deleted_at_lt(self):
        ...
        self.assertEqual(
            [self.audit2['id'], self.audit3['id']], [r.id for r in res]
        )
    ...
    def test_get_audit_list_filter_deleted_at_gte(self):
        ...
        self.assertEqual(
            [self.audit1['id'], self.audit2['id']], [r.id for r in res]
        )
...
class DbAuditTestCase(base.DbTestCase):
    def test_get_audit_list(self):
        ...
        for id_ in range(1, 4):
            audit = utils.create_test_audit(
                uuid=w_utils.generate_uuid(), name=f'My Audit {id_}'
            )
    ...
    def test_get_audit_list_with_filters(self):
        ...
        res = self.dbapi.get_audit_list(
            self.context, filters={'audit_type': 'bad-type'}
        )
        ...
        self.assertEqual(
            sorted([audit1['id'], audit2['id']]), sorted([r.id for r in res])
        )
        ...
        self.assertEqual(
            sorted([audit1['id'], audit2['id'], audit3['id']]),
    ...
    def test_destroy_audit_that_does_not_exist(self):
        self.assertRaises(
            exception.AuditNotFound, self.dbapi.destroy_audit, 1234
        )
    ...
    def test_destroy_audit_that_referenced_by_action_plans(self):
        ...
        self.assertRaises(
            exception.AuditReferenced, self.dbapi.destroy_audit, audit['id']
        )
    ...
    def test_create_same_name_audit(self):
        audit = utils.create_test_audit(
            uuid=w_utils.generate_uuid(), name='my_audit'
        )
        self.assertEqual(audit['uuid'], audit.uuid)
        self.assertRaises(
            exception.AuditAlreadyExists,
    ...
    def test_audit_status_message(self):
        ...
        self.dbapi.update_audit(audit['id'], {'status_message': 'test'})
        audit = self.dbapi.get_audit_by_id(self.context, audit['id'])
