...
class TestPrometheusBase(base.BaseTestCase):
    ...
    def setUp(self):
                        ...
                        'activeTargets': [
                            {
                                'labels': {
                                    ...
                                    'instance': '10.0.1.2:9100',
                                    'job': 'node',
                                }
                            },
                            {
                                'labels': {
                                    'fqdn': 'marios-env-again.controlplane.domain',  # noqa: E501
        ...
        self.helper._get_fqdn_label = mock.Mock(return_value='fqdn')
        ...
        self.mock_instance = mock.Mock(
            uuid='uuid-0', memory=512, disk=2, vcpus=2
        )

    def _create_helper(
        self, fqdn_label='fqdn', instance_uuid_label='resource'
    ):
        with (
            mock.patch.object(
                prometheus_base.PrometheusBase,
        ...
        helper._get_fqdn_label = mock.Mock(return_value=fqdn_label)
        return helper
    ...
    def test_prometheus_statistic_aggregation(
        self, mock_prometheus_get, mock_prometheus_query
    ):
        mock_node = mock.Mock(
            uuid='1234', hostname='marios-env.controlplane.domain'
        )
    ...
    @mock.patch.object(prometheus_client.PrometheusAPIClient, 'query')
    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_get_instance_cpu_usage(
        self, mock_prometheus_get, mock_prometheus_query
    ):
    ...
    @mock.patch.object(prometheus_client.PrometheusAPIClient, 'query')
    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_get_instance_ram_usage(
        self, mock_prometheus_get, mock_prometheus_query
    ):
        mock_instance = self.mock_instance
        ...
        ram_usage = helper.get_instance_ram_usage(
            mock_instance, period=222, aggregate="max", granularity=200
        )
    ...
    @mock.patch.object(prometheus_client.PrometheusAPIClient, 'query')
    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_get_instance_ram_allocated(
        self, mock_prometheus_get, mock_prometheus_query
    ):
        mock_instance = self.mock_instance
        helper = self._create_helper()
        ram_allocated = helper.get_instance_ram_allocated(
    ...
    @mock.patch.object(prometheus_client.PrometheusAPIClient, 'query')
    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_get_instance_root_disk_size(
        self, mock_prometheus_get, mock_prometheus_query
    ):
        mock_instance = self.mock_instance
        helper = self._create_helper()
        disk_size = helper.get_instance_root_disk_size(
            mock_instance, period=331, aggregate="avg"
        )
    ...
    @mock.patch.object(prometheus_client.PrometheusAPIClient, 'query')
    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_prometheus_stt_agg_instance_cpu_usage(
        self, mock_prometheus_get, mock_prometheus_query
    ):
        mock_instance = self.mock_instance
        expected_cpu_usage = 13.2706140350701673
    ...
    @mock.patch.object(prometheus_client.PrometheusAPIClient, 'query')
    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_prometheus_stt_agg_instance_ram_usage(
        self, mock_prometheus_get, mock_prometheus_query
    ):
        mock_instance = self.mock_instance
        expected_ram_usage = 49.86
    ...
    @mock.patch.object(prometheus_client.PrometheusAPIClient, 'query')
    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_prometheus_stt_agg_instance_root_size(
        self, mock_prometheus_get, mock_prometheus_query
    ):
        mock_instance = self.mock_instance
        helper = self._create_helper()
    ...
    @mock.patch.object(prometheus_client.PrometheusAPIClient, 'query')
    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_prometheus_stt_agg_instance_ram_alloc(
        self, mock_prometheus_get, mock_prometheus_query
    ):
        mock_instance = self.mock_instance
        helper = self._create_helper()
    ...
    def test_statistic_aggregation_metric_unavailable(self):
        self.assertRaisesRegex(
            NotImplementedError,
            'does not support statistic_series',
    ...
    def test_get_host_ram_usage_max(self):
        ram_use = self.helper.get_host_ram_usage('aNode', 456, 'max', 300)
        self.assertIsInstance(ram_use, float)
        self.mock_aggregation.assert_called_once_with(
            'aNode',
            'compute_node',
    ...
    def test_get_host_ram_usage_min(self):
        ram_use = self.helper.get_host_ram_usage('aNode', 456, 'min', 300)
        self.assertIsInstance(ram_use, float)
        self.mock_aggregation.assert_called_once_with(
            'aNode',
            'compute_node',
    ...
    def test_build_prometheus_fqdn_host_instance_map(
        ...
        expected_fqdn_list = {
            'foo.controlplane.domain',
            'bar.controlplane.domain',
            'baz.controlplane.domain',
        }
        expected_host_map = {
            'foo': 'foo.controlplane.domain',
            'bar': 'bar.controlplane.domain',
            'baz': 'baz.controlplane.domain',
        }
        helper = self._create_helper()
        self.assertEqual(helper.prometheus_fqdn_labels, expected_fqdn_list)
        self.assertEqual(
            helper.prometheus_host_instance_map, expected_host_map
        )
        ...
        mock_prometheus_get.return_value = {
            'data': {
                'activeTargets': [
                    ...
                    {'labels': {'instance': '10.1.2.2:9100', 'job': 'node'}},
                ]
            }
        }
        helper = self._create_helper()
    ...
    def test_resolve_prometheus_instance_label(self):
        ...
        result = self.helper._resolve_prometheus_instance_label('marios-env')
        self.assertEqual(result, expected_instance_label)

    @mock.patch.object(prometheus_client.PrometheusAPIClient, '_get')
    def test_resolve_prometheus_instance_label_none(self, mock_prometheus_get):
    ...
    def test_build_prometheus_query_instance_cpu_max_agg(self):
        ...
        mock_instance = mock.Mock(uuid='uuid-0', memory=512, disk=2, vcpus=4)
        result = self.helper._build_prometheus_query(
    ...
    def test_build_prometheus_query_error(self):
        self.assertRaisesRegex(
            ...
            'Cannot process prometheus meter NOPE',
        ...
        self.assertRaisesRegex(
            exception.InvalidParameter,
            'instance_label None, period 333',
            self.helper._build_prometheus_query,
            'min',
    ...
    def test_resolve_prometheus_aggregate_unknown(self):
        self.assertRaisesRegex(
            exception.InvalidParameter,
            'Unknown Watcher aggregate NOPE.',
            self.helper._resolve_prometheus_aggregate,
            'NOPE',
            'some_meter',
        )
