...
class TestStorageScope(base.TestCase):
    ...
    @mock.patch.object(cinder_helper.CinderHelper, 'get_storage_node_list')
    @mock.patch.object(
        cinder_helper.CinderHelper, 'get_volume_type_by_backendname'
    )
    ...
    def test_exclude_resources(self):
        ...
        volumes_to_exclude = []
        resources = [
            {'volumes': [{'uuid': 'VOLUME_1'}, {'uuid': 'VOLUME_2'}]},
            {
                'storage_pools': [
        ...
        self.assertEqual(
            ['PROJECT_1', 'PROJECT_2', 'PROJECT_3'], projects_to_exclude
        )
        self.assertEqual(
            ['host_0@backend_0#pool_1', 'host_1@backend_1#pool_1'],
            pools_to_exclude,
        )
