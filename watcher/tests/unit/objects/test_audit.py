...
from watcher.tests.unit.objects import utils as objutils
...
class TestAuditObject(base.DbTestCase):
        ...
        (
            ...
            dict(
                ...
                fake_audit=utils.get_test_audit(
                    created_at=timeutils.utcnow(), goal_id=goal_id
                ),
            ),
        ),
        (
    ...
    def eager_load_audit_assert(self, audit, goal):
        if self.eager:
            ...
            db_data = {
                k: v for k, v in goal.as_dict().items() if k in fields_to_check
            }
            object_data = {
                k: v
                for k, v in audit.goal.as_dict().items()
    ...
    def test_refresh(self, mock_get_audit):
        returns = [
            dict(self.fake_audit, state="first state"),
...
class TestCreateDeleteAuditObject(base.DbTestCase):
    ...
    def test_soft_delete(
        self, mock_get_audit, mock_soft_delete_audit, mock_update_audit
    ):
        mock_get_audit.return_value = self.fake_audit
    ...
    @mock.patch.object(db_api.Connection, 'get_audit_by_uuid')
    def test_destroy(self, mock_get_audit, mock_destroy_audit):
        mock_get_audit.return_value = self.fake_audit
        uuid = self.fake_audit['uuid']
...
class TestAuditObjectSendNotifications(base.DbTestCase):
    def setUp(self):
        ...
        self.fake_audit = utils.get_test_audit(
            goal_id=goal_id,
            goal=utils.get_test_goal(id=goal_id),
            strategy_id=self.fake_strategy.id,
            strategy=self.fake_strategy,
        )
    ...
    def test_send_update_notification(self, m_get_audit, m_update_audit):
        ...
        self.assertEqual(
            'audit.update', self.m_notifier.info.call_args[1]['event_type']
        )
    ...
    def test_send_create_notification(self, m_create_audit):
        ...
        self.assertEqual(1, self.m_notifier.info.call_count)
        self.assertEqual(
            'audit.create', self.m_notifier.info.call_args[1]['event_type']
        )
