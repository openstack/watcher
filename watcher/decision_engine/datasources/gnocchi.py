...
class GnocchiHelper(base.DataSourceBase):
    ...
    def statistic_aggregation(
        ...
        if aggregate == 'count':
            aggregate = 'mean'
            LOG.warning(
                'aggregate type count not supported by gnocchi,'
                ' replaced with mean.'
            )
        ...
        if resource_type == 'compute_node':
            ...
            kwargs = dict(
                query={"=": {"original_resource_id": resource_id}}, limit=1
            )
            ...
            if not resources:
                LOG.warning(
                    "The %s resource %s could not be found",
                    self.NAME,
                    resource_id,
                )
                return
        ...
        if meter_name == "instance_cpu_usage":
            if resource_type != "instance":
                LOG.warning(
                    "Unsupported resource type for metric "
                    "'instance_cpu_usage': %s",
                    resource_type,
                )
                return
            ...
            if aggregate not in ("mean", "rate:mean"):
                LOG.warning(
                    "Unsupported aggregate for instance_cpu_usage "
                    "metric: %s. "
        ...
        if statistics:
            ...
            if meter_name == "instance_cpu_usage":
                ...
                if not vcpus:
                    ...
                    vcpus = 1
                return_value *= 100 / (granularity * 10e8) / vcpus
    ...
    def get_host_cpu_usage(self, resource, period, aggregate, granularity=300):
    ...
    def get_host_ram_usage(self, resource, period, aggregate, granularity=300):
        return self.statistic_aggregation(
            resource,
            'compute_node',
            'host_ram_usage',
    ...
    def get_host_outlet_temp(
        self, resource, period, aggregate, granularity=300
    ):
        return self.statistic_aggregation(
            resource,
            'compute_node',
    ...
    def get_host_airflow(self, resource, period, aggregate, granularity=300):
        return self.statistic_aggregation(
            resource,
            'compute_node',
            'host_airflow',
    ...
    def get_host_power(self, resource, period, aggregate, granularity=300):
        return self.statistic_aggregation(
            resource,
            'compute_node',
            'host_power',
